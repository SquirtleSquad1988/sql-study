...
```md
Square brackets [] indicate optional parts a command
...
```md
Three dots ... (dots) indicate that the preceding element can be repeated
```
...
```sql
INSERT INTO <name of table> <NAME OF ROW (in caps)>
```
...
```sql
DELETE FROM <table name> WHERE <row identifier>
DELETE is the answer you're looking for
```
